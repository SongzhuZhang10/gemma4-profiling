...
if ! command -v cmake >/dev/null 2>&1 || ! cmake --version >/dev/null 2>&1; then
  workflow_log "A usable cmake launcher was not found in PATH. Installing a user-space cmake wheel into $DL_ENV_ROOT."
  run_pip install --upgrade --force-reinstall --no-cache-dir 'cmake>=3.28,<4'
fi
