...
if [[ $# -ne 1 ]]; then
  workflow_log "Usage: bash 07_open_report.sh <report-path>"
  workflow_log "Examples:"
  workflow_log "  bash 07_open_report.sh artifacts/reports/prefill.ncu-rep"
  workflow_log "  bash 07_open_report.sh artifacts/reports/decode.nsys-rep"
  exit 1
fi
...
if [[ "$viewer" == "ncu-ui" ]]; then
  ...
  # Force a fresh window so large reports are not silently handed to an
  # existing hidden/shared instance.
  viewer_args+=(--shared-instance off --no-splash)

  if [[ -n "${WAYLAND_DISPLAY:-}" ]]; then
    # Under WSLg, Nsight Compute's Qt Wayland backend can fail to show a
    # window even though the process starts. XWayland/xcb is reliable here.
